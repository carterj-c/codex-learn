...
If `curricula/<subject>/sources/catalog.md` exists, give the subagent the catalog, `sources/design-brief.md` when present, and only the source entries relevant to the proposed curriculum. The learner decides the source role; never infer it from the catalog.

Ask for a source decision before drafting when cataloged sources exist but neither the learner nor the design brief says whether to:
