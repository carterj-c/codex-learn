...
On an explicit approval, use the same custom subagent to create or revise the curriculum files and a blank lesson index. Preserve observed evidence, existing lessons, and artifacts when revising. The generic teacher takes over only in a later teaching task when the learner invokes `$curriculum` or a subject entry skill.
